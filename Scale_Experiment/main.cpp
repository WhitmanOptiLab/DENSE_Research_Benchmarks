...
template<class Scale_Simulation>
void simulate_scale(int ac, char** av, Static_Args* args, std::string type){
  ...
  for(auto& entry : names_and_analysis){
    entry.first = "Scale_Experiment/" + type + ".csv";
  }
...
int main(int argc, char* argv[]){
  ...
  simulate_scale<Fast_Gillespie_Direct_Simulation>(ac, av, &args, "Fast_Gillespie_Scale");
  
  std::cout << style::apply(Color::yellow) << "Finished the Fast Gillespie Simulation\n\n";
  std::cout << "Starting the Deterministic Simulation\n" << style::reset();
  
  simulate_scale<Deterministic_Simulation>(ac, av, &args, "Deterministic_Scale");
